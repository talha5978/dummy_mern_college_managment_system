...
>(({ className, ...props }, ref) => (
	<ComboboxPrimitive.Root data-slot="combobox" ref={ref} className={cn(className)} {...props} />
...
>(({ className, children, ...props }, ref) => (
	<ComboboxPrimitive.BadgeItem
		...
		{...props}
	>
		<span className="truncate text-sm text-secondary-foreground">{children}</span>
		<ComboboxPrimitive.BadgeItemDelete
			data-slot="combobox-badge-item-delete"
			className="shrink-0 rounded p-0.5 opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring data-highlighted:bg-destructive cursor-pointer"
		>
			<X className="h-4 w-4" />
		</ComboboxPrimitive.BadgeItemDelete>
	</ComboboxPrimitive.BadgeItem>
...
>(({ className, children, outset, ...props }, ref) => (
	...
	>
		<ComboboxPrimitive.ItemIndicator
			className={cn("absolute flex h-3.5 w-3.5 items-center justify-center", outset ? "right-2" : "left-2")}
		>
