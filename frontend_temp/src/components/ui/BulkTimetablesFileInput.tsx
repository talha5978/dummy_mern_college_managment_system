...
import { useAddTimetableInBulkMutation } from "../../services/timetables.api";
...
export function FileUploadInputForm() {
	...
	const [addTimetablesInBulk, { isLoading: isAdding }] = useAddTimetableInBulkMutation();

	const onSubmit = async (data: FormValues) => {
		console.log("Submitted data:", data.file);

		const formData = new FormData();
		formData.append("timetable", data.file);
		
		try {
			const res = await addTimetablesInBulk(formData).unwrap();
			if (res?.success) {
	...
	const handleDownload = () => {
		window.open('/api/timetables/bulk/sample', "_blank");
	};
